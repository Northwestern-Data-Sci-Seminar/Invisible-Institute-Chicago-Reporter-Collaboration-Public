...
def load_data(path = data_path):
    ...
    df2 = df2.dropna()
    X = df2.drop(["id", "allegation_count"], axis = 1)
...
def createLinearRegressionModel(X, y):
    ...
    print(cv_scores)
    print("Linear Regression (with salary and num_awards) average 5-fold CV R2 score: {}".format(np.mean(cv_scores)))
...
def createGBTModel(X, y):
    ...
    print(cv_scores)
    print("GBT Regression (with salary and num_awards)  average 5-fold CV R2 score: {}".format(np.mean(cv_scores)))
